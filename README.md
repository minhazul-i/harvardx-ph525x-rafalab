...
* **Populations, Samples and Estimates:** A population is considered fixed, while the randomness comes from which individuals we sample from it. Population values are represented with lowercase letters (\(x_1,\ldots,x_m\)), while random sample values are represented with uppercase letters (\(X_1,\ldots,X_M\)). Population parameters such as \(\mu\) and \(\sigma\) describe the entire population and are usually unknown in practice. We therefore take a random sample and calculate statistics such as the sample mean \(\bar X\), which estimates the population mean \(\mu\). Because different random samples give different sample means, \(\bar X\) is itself a random variable. Similarly, \(\bar Y-\bar X\) is a random variable used to estimate the population difference \(\mu_Y-\mu_X\). The Central Limit Theorem will allow us to mathematically describe the sampling distribution of these estimates.
